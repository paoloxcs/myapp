	...
	<div class="row mt-5">
		...
		<section class="col-xs-12 col-sm-12 col-md-9">
			...
			<div class="row mt-3 d-flex flex-wrap">
				@foreach($category->products as $indexprod => $product)
				<section class="profile">
					<a href="{{url('productos/'.$category->slug.'/'.$product->slug)}}">
						...
						<div class="row mt-2">
							<ul class="nav nav-tabs" id="myTab{{$indexprod}}" role="tablist">
								@foreach($product->operating_conditions as $index => $opera)
									@if($index==0)
										<li class="nav-item">
											<a class="nav-link active" id="home-tab{{$index}}" data-toggle="tab" href="#{{$opera->measurement->sigla}}{{$product->id}}" role="tab" aria-controls="home" aria-selected="true"><small>{{$opera->measurement->sigla}}</small></a>
										</li>
									@else
										<li class="nav-item">
											<a class="nav-link" id="profile-tab{{$index}}" data-toggle="tab" href="#{{$opera->measurement->sigla}}{{$product->id}}" role="tab" aria-controls="profile" aria-selected="false"><small>{{$opera->measurement->sigla}}</small></a>
										</li>
									@endif
								@endforeach
							</ul>

							<div class="tab-content" id="myTabContent{{$indexprod}}">

							@foreach($product->operating_conditions as $index2 => $content)
								@if($index2==0)
									<div class="tab-pane show active" id="{{$content->measurement->sigla}}{{$product->id}}" role="tabpanel" aria-labelledby="home-tab">
								...
								@else
									<div class="tab-pane" id="{{$content->measurement->sigla}}{{$product->id}}" role="tabpanel" aria-labelledby="home-tab">
										<table class="table table-sm table-condensed table-borderless">
											<tbody>
												<tr>
													<td><small>Presión Máxima</small></td>
